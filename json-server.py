...
class JSONServer(HandleRequests):
    ...
    def do_POST(self):
        ...
        if url["requested_resource"] == "ships":
            ...
            posted_item = add_ship(request_body)
            if posted_item:
                return self.response(
                    json.dumps(posted_item), status.HTTP_201_SUCCESS_CREATED.value
                )

        elif url["requested_resource"] == "docks":

            posted_item = add_dock(request_body)
            if posted_item:
                return self.response(
                    json.dumps(posted_item), status.HTTP_201_SUCCESS_CREATED.value
                )
        ...
        if url["requested_resource"] == "haulers":
            ...
            posted_item = add_hauler(request_body)
            if posted_item:
                return self.response(
                    json.dumps(posted_item), status.HTTP_201_SUCCESS_CREATED.value
                )
